...
import { getChats as onGetChats, deleteSession, renameSession } from "../../slices/chats/thunk";
...
const ChatList: React.FC<Props> = ({ userChatOpen, currentSessionId }) => {
  ...
  const selectProperties = createSelector(
    (state: { chats: { chats: ChatSession[]; loading: boolean } }) => state.chats,
    (chats) => ({
      chats: chats.chats,
  ...
  const handleRenameSubmit = async (chat: ChatSession) => {
    if (chat.session_id && editingName.trim()) {
      await dispatch(renameSession({ sessionId: chat.session_id, newName: editingName.trim() }));
        ...
        <div className="chat-leftsidebar-nav flex-grow-1">
          <div className="px-2 pt-4 pb-4">
            <div className="d-flex align-items-center mb-4">
              <h5 className="font-size-14 mb-0">General Assist</h5>
              <div className="d-flex align-items-center ms-auto">
                ...
                <div className="position-relative search-box">
                  ...
                  />
                  <i className="bx bx-search search-icon position-absolute" 
                ...
                <SimpleBar style={{ height: "calc(100vh - 180px)" }}>
                  {chats.map((chat, index) => (
                    <li 
                      ...
                      className={`mb-2 ${chat.session_id === currentSessionId ? "active" : ""}`}
                    >
                      <div className="d-flex align-items-center">
                        ...
                        >
                          ...
                          <div className="flex-grow-1 overflow-hidden text-truncate">
                            {editingChatId === chat.session_id ? (
                              ...
                              <h5 className="text-truncate font-size-14 mb-0" style={{ maxWidth: "200px" }}>
                                {chat.session_name || "New Chat"}
                              </h5>
                            )}
                          </div>
                        </Link>
                        {chat.session_id && (
                          <Dropdown
                            ...
                            className="ms-2"
                          >
                            <DropdownToggle color="none" className="p-0 border-0">
                              <i className="bx bx-dots-vertical-rounded fs-4 text-muted"></i>
                            </DropdownToggle>
                            <DropdownMenu className="dropdown-menu-end">
                              <DropdownItem onClick={() => handleRename(chat)}>
                                <i className="bx bx-edit-alt me-2"></i> Rename
                              </DropdownItem>
                              <DropdownItem onClick={() => handleDelete(chat.session_id!)}>
